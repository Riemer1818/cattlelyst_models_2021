...
% Function to sample parameter space for the model extension 6.1.

% N = number of parameter samples to simulate in the chunk
%% Optimise model extension 6.1 function
function [output] = optimise_model_e6_2(p_sets,N,i)
...
for i = 1:N
    ...
    for j = 1:2
        ...
        if j == 1 % 50% strength input
            [t,y] = ode15s(@model_e6_2,time,ics,[],F(i)*0.5,ts1on(i),ts1off(i),p_sets(i,:));
            
        elseif j == 2 % 100% strength input
            [t,y] = ode15s(@model_e6_2,time,ics,[],F(i),ts1on(i),ts1off(i),p_sets(i,:));
