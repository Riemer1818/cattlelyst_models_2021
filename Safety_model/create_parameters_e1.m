%% Goal: create random parameter sets for model extension 1 using latin hypercube sampling
%Latin hypercube creates random numbers between 0-1. 
%N = number of parameter samples to simulate
%this function was run three times. Once with the normal bounds saved as
%*_o.mat, once with b10=0 saved as *_o1.mat and once with k8=0 saved as
%*_o2.mat
%% Function to create parameter sets
function create_parameters_e1(N)
    ...
    % New parameters
    % Parameter b8
    p_sets(:,23) = average_b_m*10.^((2*p_sets(:,23))-1);
    % Parameter b9
    p_sets(:,24) = average_b_p*10.^((2*p_sets(:,24))-1);
    % Parameter b10
    p_sets(:,25) = average_b_p*10.^((2*p_sets(:,25))-1);
    %p_sets(:,25) = 0;
    % Parameter g3
    p_sets(:,26) = average_g*10.^((2*p_sets(:,26))-1);
    % Parameter k7
    p_sets(:,27) = median_lowhoksok(1,18)*10.^((2*p_sets(:,27))-1);
    % Parameter k8
    p_sets(:,28) = median_lowhoksok(1,19)*10.^((2*p_sets(:,28))-1);
    %p_sets(:,28) = 0;

    %save p-sets
    save('e1_parameters_o.mat','p_sets')
