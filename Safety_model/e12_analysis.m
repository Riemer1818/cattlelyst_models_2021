%% file used to analyse the output of the simulations of the model extension 1(2) (k8=0)
% contains:
...
% 4. Time series plot e1(2)

% input files: 
%     e1_output_m2_1.mat
%     e1_output_o2_1.mat
%     e1_parameters_o2.mat
%     median_low_hoksokratio2.mat
% output files:
%     e12_max_sensitivity_analysis.mat
%% 1. create a matrix with end concentrations for the parameter sets that have the median p-set of the full model as first 22 parameter values
% load output file median parameter set
load('e1_output_m2_1.mat','sim')
...
% ratio [F]high + hoksok ratio [F]low/hoksok ratio [F]high.
    ...
    sortB = sortrows(B_high_F_e,16);
    C_high_F_e = [B_high_F_e A_low_F_e(:,12)./A_high_F_e(:,12) A_low_F_e(:,7)./A_high_F_e(:,7)];
%% find the parameter sets with a the max sensitivity + corresponding timepoint
load('e1_output_o2_1.mat','sim')
...
save('e12_max_sensitivity_analysis.mat','sort_max')
%% Time series plot e1(2)
load('median_low_hoksokratio2','params_GhighF_ratio');
load('e1_parameters_o2','p_sets')
N = 1;
q = 1;

% select parameter set to simulate 
p_set = 3225; %extension 1(2)
p_set_m = 1056; %original biosafety circuit

% make simulation of given paremeter set for extension 1(2) (new) and original
...
sgtitle('Time series extension 1 (unbinding rate C-FrmR=0) - p-set with highest sensitivity at t=48h','FontSize',22);
